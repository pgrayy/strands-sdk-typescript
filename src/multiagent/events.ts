 ...
 *
 * Use `inner.source` to determine the event origin, then `inner.type`
 * to switch on the specific event within that source's event union.
 */
export type NodeInnerEvent =
...
export class NodeStreamUpdateEvent extends HookableEvent {
  ...
  readonly state: MultiAgentState
  readonly inner: NodeInnerEvent

  constructor(data: { nodeId: string; nodeType: NodeType; state: MultiAgentState; inner: NodeInnerEvent }) {
    super()
    this.nodeId = data.nodeId
